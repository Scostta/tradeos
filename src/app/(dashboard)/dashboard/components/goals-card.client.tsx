...
import { updateGoals } from "~/actions/goals"
...
export function GoalsCard({ goals, progress }: { goals: Goals; progress: GoalsProgress }): ReactElement {
  ...
  function handleSave() {
    ...
    startTransition(async () => {
      ...
      if (result.success) {
        showToast(GOALS.EDITOR.SUCCESS, "success")
        setIsOpen(false)
        router.refresh()
      } else {
        showToast(GOALS.EDITOR.ERROR, "error")
      }
    })
  }
    ...
    <>
      ...
      <div className="card p-4 flex flex-col gap-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <span className="text-md font-semibold text-text">{GOALS.TITLE}</span>
            <span className="mono text-xs text-text-mute">{progress.monthLabel}</span>
          </div>
          <button onClick={openModal} className="text-xs text-text-mute hover:text-text-dim transition-colors">
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {p && (
              <Metric
                ...
                pct={p.pct}
                color={p.met ? "var(--color-profit)" : "var(--color-accent)"}
              />
            )}
            {w && (
              <Metric
                label={GOALS.WIN_RATE}
                ...
                pct={w.pct}
                color={w.met ? "var(--color-profit)" : "var(--color-accent)"}
              />
            )}
            {d && (
              <Metric
                ...
                pct={d.pct}
                color={d.breached ? "var(--color-loss)" : d.pct >= 0.8 ? "#f59e0b" : "var(--color-profit)"}
              />
            )}
            {td && (
              <Metric
                label={GOALS.TRADING_DAYS}
                ...
                pct={td.pct}
                color={td.met ? "var(--color-profit)" : "var(--color-accent)"}
              />
            )}
          </div>
        )}
      ...
      {isOpen && createPortal(
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setIsOpen(false)}>
          <div
            className="card border-border-hi w-full p-6 flex flex-col gap-5"
            style={{ maxWidth: 440, boxShadow: "0 20px 80px rgba(0,0,0,0.6)" }}
            onClick={e => e.stopPropagation()}
          >
            <div className="flex items-start justify-between gap-3">
              <div>
                <h2 className="text-lg font-semibold text-text">{GOALS.EDITOR.TITLE}</h2>
