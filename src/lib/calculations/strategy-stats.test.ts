...
function strat(id: string, name: string): Strategy {
  return {
...
function trade(over: Partial<Trade>): Trade {
  return {
    ...
    strategyId: "s1", session: null, notes: null, tags: null,
    createdAt: "2026-06-10T15:30:00.000Z", ...over,
  }
}

describe("computeStrategyStats", () => {
  it("aggregates per-strategy stats including R via the account risk fallback", () => {
    const strategies = [strat("s1", "Breakout"), strat("s2", "Fade")]
    const trades = [
      trade({ netPnl: 1000 }),                       // s1, R = +2 (risk 500)
      trade({ netPnl: -500 }),                       // s1, R = −1
      trade({ netPnl: 999, strategyId: null }),      // unassigned — ignored
    ]
    const risk = new Map<string, number | null>([["a1", 500]])

    const [s1, s2] = computeStrategyStats(strategies, trades, risk)
